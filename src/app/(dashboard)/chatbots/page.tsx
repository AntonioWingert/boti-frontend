...
import { Plus, Search, Edit, Trash2, Bot, Play, Pause, Settings, Workflow } from 'lucide-react'
...
export default function ChatbotsPage() {
  ...
  const fetchChatbots = async () => {
    try {
      setLoading(true)
      // Buscar apenas chatbots da empresa do usuário (exceto ADMINs)
      const url = isAdmin() ? '/chatbots' : `/chatbots?companyId=${user?.companyId}`
      const response = await api.get<Chatbot[]>(url)
        ...
        const reactFlowData = {
          nodes: flow.nodes?.map((node: any) => {
            ...
            const nodeData = {
              ...
              // Preservar propriedades originais do backend
              nodeType: node.nodeType,
              isStart: node.isStart,
              isEnd: node.isEnd,
              realId: node.id, // ID real do banco
          ...
          edges: (() => {
            const allEdges = flow.nodes?.flatMap((node: any) => {
              const nodeConnections = []
              
              // Conexões diretas do nó (apenas para nós MESSAGE)
              if (node.outgoingConnections && node.nodeType === 'MESSAGE') {
                const mainConnections = node.outgoingConnections.map((conn: any) => ({
                  id: conn.id,
                ...
                nodeConnections.push(...mainConnections)
              }
              
              // Para nós OPTION, processar outgoingConnections se as opções não têm targetNodeId
              if (node.nodeType === 'OPTION') {
                // Se as opções não têm targetNodeId, usar outgoingConnections
                const hasOptionsWithTarget = node.options?.some((opt: any) => opt.targetNodeId)
                if (!hasOptionsWithTarget && node.outgoingConnections) {
                  const optionConnections = node.outgoingConnections.map((conn: any, index: number) => ({
                    id: conn.id,
                    source: conn.sourceNodeId,
                    target: conn.targetNodeId,
                    sourceHandle: `option-${index}`, // Handle específico da opção
                    targetHandle: 'top',
                    style: { stroke: '#f59e0b', strokeWidth: 3 }, // Cor laranja para opções
                    animated: true
                  }))
                  nodeConnections.push(...optionConnections)
                }
              }
                ...
                node.options.forEach((option: any, index: number) => {
                  if (option.targetNodeId) {
                    ...
                    const connection = {
                      ...
                      animated: true,
                      label: option.text, // Label com o texto da opção
                      labelStyle: { 
                        fill: '#f59e0b', 
                        fontWeight: 'bold',
                        fontSize: '12px'
            ...
            // Remover conexões duplicadas baseadas em source + target + sourceHandle + targetHandle
            const uniqueEdges = []
            const seenConnections = new Set()
            
            for (const edge of allEdges) {
              const connectionKey = `${edge.source}-${edge.target}-${edge.sourceHandle}-${edge.targetHandle}`
  ...
  const handleFlowEditorSave = async (flow: any) => {
    ...
    try {
      ...
      try {
        ...
        console.log('ℹ️ Nenhum nó existente encontrado ou erro ao buscar:', (error as any).message)
      }
      ...
      const nodeIdMap = new Map<string, string>()
        ...
        for (const node of flow.nodes) {
          ...
          const existingNode = existingNodes.find((n: any) => n.id === node.id)
      ...
      const reactFlowNodeIds = new Set(flow.nodes.map((n: any) => n.id))
  ...
  return (
    <div className="space-y-4 sm:space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
        <div>
          <h1 className="text-2xl sm:text-3xl font-bold text-boti-text font-poppins">Chatbots</h1>
          <p className="text-gray-600">Gerencie os chatbots da sua empresa</p>
        </div>
        <div className="flex flex-col sm:flex-row gap-2">
          <Button onClick={fetchChatbots} variant="outline" className="w-full sm:w-auto">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>
            Encontre chatbots por nome ou descrição
          </CardDescription>
        </CardHeader>
        <CardContent>
      ...
      <div className="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {filteredChatbots.map((chatbot) => (
          <Card key={chatbot.id} className="hover:shadow-lg transition-shadow min-h-[280px] flex flex-col">
            <CardHeader className="pb-3">
              <div className="flex justify-between items-start">
                <div className="flex items-center space-x-2">
                  <Bot className="h-5 w-5 text-boti-primary" />
              ...
              <div className="text-sm text-gray-500 mb-4 space-y-1">
                ...
                {chatbot.autoEndMessage && (
                  <div className="mt-2 p-2 bg-blue-50 border border-blue-200 rounded-md">
                    <p className="text-xs text-blue-600 font-medium">Mensagem de Finalização:</p>
                    <p className="text-xs text-blue-700 mt-1">{chatbot.autoEndMessage}</p>
                  </div>
                )}
              </div>
              ...
              <div className="flex flex-col sm:flex-row gap-2 pt-4 border-t mt-auto">
                ...
                <div className="flex gap-2">
                  ...
                  <Button
                    ...
                    className="flex-1 text-boti-primary hover:text-boti-primary/80"
                  >
